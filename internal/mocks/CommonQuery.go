// Code generated by mockery v2.43.2. DO NOT EDIT.

package mocks

import (
	context "context"

	gorm "gorm.io/gorm"

	mock "github.com/stretchr/testify/mock"

	model "books-api/internal/model"
)

// CommonQuery is an autogenerated mock type for the CommonQuery type
type CommonQuery[T interface{}] struct {
	mock.Mock
}

// CreateTx provides a mock function with given fields: ctx, tx, data
func (_m *CommonQuery[T]) CreateTx(ctx context.Context, tx *gorm.DB, data *T) error {
	ret := _m.Called(ctx, tx, data)

	if len(ret) == 0 {
		panic("no return value specified for CreateTx")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, *T) error); ok {
		r0 = rf(ctx, tx, data)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// DeleteByIDTx provides a mock function with given fields: ctx, tx, id
func (_m *CommonQuery[T]) DeleteByIDTx(ctx context.Context, tx *gorm.DB, id string) error {
	ret := _m.Called(ctx, tx, id)

	if len(ret) == 0 {
		panic("no return value specified for DeleteByIDTx")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, string) error); ok {
		r0 = rf(ctx, tx, id)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Find provides a mock function with given fields: ctx, tx, order, filter
func (_m *CommonQuery[T]) Find(ctx context.Context, tx *gorm.DB, order model.OrderParam, filter model.FilterParams) (*[]T, error) {
	ret := _m.Called(ctx, tx, order, filter)

	if len(ret) == 0 {
		panic("no return value specified for Find")
	}

	var r0 *[]T
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, model.OrderParam, model.FilterParams) (*[]T, error)); ok {
		return rf(ctx, tx, order, filter)
	}
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, model.OrderParam, model.FilterParams) *[]T); ok {
		r0 = rf(ctx, tx, order, filter)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*[]T)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, *gorm.DB, model.OrderParam, model.FilterParams) error); ok {
		r1 = rf(ctx, tx, order, filter)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FindByFilter provides a mock function with given fields: ctx, tx, filter, order
func (_m *CommonQuery[T]) FindByFilter(ctx context.Context, tx *gorm.DB, filter model.FilterParams, order model.OrderParam) (*T, error) {
	ret := _m.Called(ctx, tx, filter, order)

	if len(ret) == 0 {
		panic("no return value specified for FindByFilter")
	}

	var r0 *T
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, model.FilterParams, model.OrderParam) (*T, error)); ok {
		return rf(ctx, tx, filter, order)
	}
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, model.FilterParams, model.OrderParam) *T); ok {
		r0 = rf(ctx, tx, filter, order)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*T)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, *gorm.DB, model.FilterParams, model.OrderParam) error); ok {
		r1 = rf(ctx, tx, filter, order)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FindByID provides a mock function with given fields: ctx, tx, id
func (_m *CommonQuery[T]) FindByID(ctx context.Context, tx *gorm.DB, id string) (*T, error) {
	ret := _m.Called(ctx, tx, id)

	if len(ret) == 0 {
		panic("no return value specified for FindByID")
	}

	var r0 *T
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, string) (*T, error)); ok {
		return rf(ctx, tx, id)
	}
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, string) *T); ok {
		r0 = rf(ctx, tx, id)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*T)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, *gorm.DB, string) error); ok {
		r1 = rf(ctx, tx, id)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FindByPagination provides a mock function with given fields: ctx, tx, page, order, filter
func (_m *CommonQuery[T]) FindByPagination(ctx context.Context, tx *gorm.DB, page model.PaginationParam, order model.OrderParam, filter model.FilterParams) (*model.PaginationData[T], error) {
	ret := _m.Called(ctx, tx, page, order, filter)

	if len(ret) == 0 {
		panic("no return value specified for FindByPagination")
	}

	var r0 *model.PaginationData[T]
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, model.PaginationParam, model.OrderParam, model.FilterParams) (*model.PaginationData[T], error)); ok {
		return rf(ctx, tx, page, order, filter)
	}
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, model.PaginationParam, model.OrderParam, model.FilterParams) *model.PaginationData[T]); ok {
		r0 = rf(ctx, tx, page, order, filter)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.PaginationData[T])
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, *gorm.DB, model.PaginationParam, model.OrderParam, model.FilterParams) error); ok {
		r1 = rf(ctx, tx, page, order, filter)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// UpdateAssociationMany2ManyTx provides a mock function with given fields: tx, data
func (_m *CommonQuery[T]) UpdateAssociationMany2ManyTx(tx *gorm.DB, data *T) error {
	ret := _m.Called(tx, data)

	if len(ret) == 0 {
		panic("no return value specified for UpdateAssociationMany2ManyTx")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(*gorm.DB, *T) error); ok {
		r0 = rf(tx, data)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// UpdateTx provides a mock function with given fields: ctx, tx, data
func (_m *CommonQuery[T]) UpdateTx(ctx context.Context, tx *gorm.DB, data *T) error {
	ret := _m.Called(ctx, tx, data)

	if len(ret) == 0 {
		panic("no return value specified for UpdateTx")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, *T) error); ok {
		r0 = rf(ctx, tx, data)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// UpdateTxWithAssociations provides a mock function with given fields: ctx, tx, data
func (_m *CommonQuery[T]) UpdateTxWithAssociations(ctx context.Context, tx *gorm.DB, data *T) error {
	ret := _m.Called(ctx, tx, data)

	if len(ret) == 0 {
		panic("no return value specified for UpdateTxWithAssociations")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *gorm.DB, *T) error); ok {
		r0 = rf(ctx, tx, data)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// NewCommonQuery creates a new instance of CommonQuery. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewCommonQuery[T interface{}](t interface {
	mock.TestingT
	Cleanup(func())
}) *CommonQuery[T] {
	mock := &CommonQuery[T]{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
